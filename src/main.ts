...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function openProjectModal(index: number) {
        ...
        if (!modalOverlay || !pageLeft || !pageRight) return;
        ...
        if (leftCat) leftCat.textContent = 'PROJECT';
        if (leftName) leftName.textContent = project.title;
        if (leftSub) leftSub.textContent = project.timeline || '';
        ...
        // Populate right page with project details
        if (pageRight) {
            let techHTML = '';
    ...
    // Build comprehensive system prompt with accurate experience data
    function buildSystemPrompt() {
        const experienceList = comprehensiveKnowledge.experience.map((exp, idx) =>
            `${idx + 1}. **${exp.role}** at ${exp.company} (${exp.duration})
...
Example: For "associate SE role requiring ticket completion and testing" → Highlight Webvine experience specifically (completed scoped tickets end to end, wrote and ran UAT)`;
    ...
    // Gemini AI integration with proper context
